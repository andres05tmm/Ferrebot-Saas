 ...
 */
import { Truck, Building2, User } from '@/lib/icons.jsx'
...
export default function EstadoMaquinas({ maquinas }) {
  ...
  return (
    <SeccionPanel
      icon={Truck}
      titulo="Máquinas"
      accion={<span className="text-caption tabular-nums text-muted-foreground">{total} en flota</span>}
    ...
    >
      <div className="flex flex-wrap gap-1.5 px-4 py-3">
        ...
        {ORDEN.every((e) => !porEstado[e]) && (
          <span className="text-meta text-muted-foreground">Sin máquinas registradas.</span>
        )}
      </div>

      <div className="border-t border-border-subtle">
        <div className="px-4 pt-2.5 pb-1 text-micro uppercase tracking-wider text-muted-foreground">
          Ocupadas hoy {ocupadas.length > 0 && <span className="tabular-nums">· {ocupadas.length}</span>}
        </div>
        {ocupadas.length === 0 ? (
          <p className="px-4 pb-3.5 text-meta text-muted-foreground">Ninguna máquina en obra hoy.</p>
        ) : (
  ...
  return (
    <li className="flex items-center gap-3 px-4 py-2.5">
      <div className="min-w-0 flex-1">
        <div className="truncate text-body-sm font-medium text-foreground">{maquina.maquina}</div>
      ...
      <div className="shrink-0 text-right">
        {conParte ? (
          <>
            <div className="text-body-sm font-semibold tabular-nums text-foreground">{cop(n(maquina.ingreso_hoy))}</div>
            <div className="text-caption tabular-nums text-muted-foreground">{num(maquina.horas_hoy)} h hoy</div>
          </>
        ) : (
          <div className="text-caption font-medium text-warning">— sin parte hoy</div>
        )}
      </div>
    </li>
  )
