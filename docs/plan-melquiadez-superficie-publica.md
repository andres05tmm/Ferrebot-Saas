...
> Estado: **Propuesto** (12 jun 2026). Cubre: rebrand a **Melquiadez**, landing nueva con sign-in
...
- **Landing actual**: `landing/index.html` (810 líneas, HTML a mano, marca FerreBot), desplegada como
  assets estáticos de Cloudflare (`wrangler.jsonc`, ya nombrada `melquiadez-landing`).
...
- ⚠️ **Cambio de código requerido (verificado contra `resolver.py`)**: `_slug_from_host` retorna el
  subdominio **incondicionalmente** — con wildcard, `app.melquiadez.com` resolvería al tenant "app" y
  NUNCA caería al claim del JWT. Hay que añadir una lista de **labels reservados**
  (`app`, `api`, `www`, `admin`) que `_slug_from_host` trate como "sin subdominio" (return `None`),
  + prohibir esos slugs en el manifiesto (`tools/manifest/schema.py`) + test.
...
- **v2 (pulido)**: endpoint `POST /auth/demo {vertical}` rate-limited que emite un JWT de vida corta
  (30 min) SOLO para tenants marcados `es_demo` en el control DB. Evita credenciales públicas.
...
> Verificado contra `core/tenancy/catalogo.py`: `pack_pedidos` y `pack_reservas` ya existen en el
> catálogo con sus dependencias (`pos` y `pack_agenda`). Confirmar qué tienen implementado en el
> registry de packs (`tools/manifest/packs/registry.py` hoy solo registra `pack_agenda`/`pack_faq`)
> — el loader de datos demo de pedidos/reservas puede ser parte de M5.
...
- **Resolver con labels reservados**: el fix de `_slug_from_host` (§3) va ANTES de prender el
  wildcard en DNS — sin él, `app.melquiadez.com` rompe.
- **Pack pedidos/reservas a medias**: catálogo ✓, registry de packs incompleto — dimensionar en M5.
- **CORS**: abrir SOLO `/auth/login` (y quizá `/auth/reset/solicitar`) al origin de la landing;
...
- [ ] M1 — Logo + tokens + de-branding.
- [ ] M2 — DNS/Railway/`BASE_DOMAIN`/slugs reservados.
- [ ] M3 — Landing React con catálogo 21st.dev.
- [ ] M4 — `/login` en landing + CORS + handoff.
- [ ] M5 — 4 tenants demo provisionados y sembrados.
- [ ] M6 — Presets de branding por vertical.
- [ ] M7 — `tools/switch_demo.py` + rebrand Kapso.
- [ ] M8 — Checklist onboarding + smoke E2E + QA visual.
