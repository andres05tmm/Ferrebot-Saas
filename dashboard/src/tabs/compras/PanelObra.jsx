/*
 * PanelObra — registro DIRECTO de compras (el tab Compras de antes de la reforma del ciclo).
 *
 * Sigue vivo por dos razones: es el flujo del vertical construcción (compras imputadas a obra y
 * viajes de material, que NO son pedidos de mercancía ni mueven inventario) y es el fallback para
 * los tenants que no tienen la feature `pedidos_proveedor`. Se movió tal cual desde TabCompras.jsx.
 *
...
export default function PanelObra() {
  const { isAdmin } = useAuth()
  // Registrar una compra directa es admin-only en el backend (POST /compras).
  if (!isAdmin()) {
    return (
      <Card className="p-8 text-center text-sm text-muted-foreground">
        Registrar compras es solo para administradores.
      </Card>
    )
  }
